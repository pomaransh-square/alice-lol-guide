...
export class LeagueOfLegendsResponseFormatter {
    ...
    help = () => reply`
        ...
        К примеру, вы можете сказать "Пайк руны", и я перечислю ${effect('megaphone')} ТОЛЬКО ${effect('-')} руны. ${br()}
        Если я вам помогла, и более от меня ничего не нужно, просто скажите "${['Остановись', 'Хватит', 'Перестань']}, пожалуйста"
        ${secretHelp}
        ${buttons(this.buttons)}
    `;
